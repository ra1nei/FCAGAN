...
from .lpips import LPIPS  # ✅ thêm LPIPS
...
class Evaluator():
    def __init__(self, opt, num_classes=None, text2label=None):
        ...
        self.out_root = os.path.join(opt.results_dir, opt.name, opt.phase + f'_{opt.epoch}', 'metrics', opt.evaluate_mode)
        ...
        self.criterionLPIPS = LPIPS(net='vgg').to(self.device)  # ✅ LPIPS metric
    ...
    def compute_lpips(self):
        # LPIPS expects [-1,1] input range
        self.lpips = self.criterionLPIPS(self.generated_images, self.gt_images).item()  # ✅ compute LPIPS
    ...
    def record_current_results(self):
        ...
        print('lpips       :', self.lpips)  # ✅ add LPIPS
        print('fid         :', self.fid, '\n')
    ...
    def compute_final_results(self):
        num_images, num_correct, l1, ssim, msssim, lpips, fid = 0, 0, 0, 0, 0, 0, 0
        files = os.listdir(self.out_root)
        print('loading metrics...')
        for file in files:
            if '.txt' not in file or file == 'final_results.txt':
                continue
            ...
            with open(file_path, 'r') as f:
                ...
                lpips += float(l[5]) * int(l[0])  # ✅ add LPIPS
                fid += float(l[6]) * int(l[0])
...
class EvaluatorDataset(Dataset):
    def __init__(self, opt):
        data_root = os.path.join(opt.results_dir, opt.name, opt.phase + f'_{opt.epoch}', 'images')
        if opt.evaluate_mode == 'content':
            part = 1
        elif opt.evaluate_mode == 'style':
            part = 0
        else:
            raise ValueError("evaluate_mode must be 'content' or 'style'")
    ...
    def __getitem__(self, idx):
        ...
        gt_images = torch.cat([self.load_image(p) for p in gt_images], 0)
        generated_images = torch.cat([self.load_image(p) for p in generated_images], 0)
